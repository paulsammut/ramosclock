...
static unsigned char CC1101_config_DefusePanel[NR_OF_CONFIG_REGISTERS] = {
    ...
    0x65,
    0x07,
    0x3B, //MCSM1 0x30 is default.FOr Master 3B, For slave, it will be 3F which is say in RF mode always!
    0x18, //18 is default
    0x16,
    0x6C,
    0x07,
...
void init_CC1101(void) {
    ANSELbits.ANS0 = 0; //digital input buffer for the GD00 pin enabled
    ANSELbits.ANS1 = 0; //digital input buffer for the GD02 pin enabled
    ...
    //turn on the chip
    TRIS_RF_POW = 0; //outputs

    RF_POW = 0xFF; //turn it on
    Delay10KTCYx(100);
    //from the datasheet
    ...
    while (PORTCbits.RC4); //wait to be pulled low
